...
//or filtering 
  ...
  Map<String, double> calculateDailyAverages() {
    ...
    print('Total assets being processed: ${assets.length}');
    
    for (var asset in assets) {
      if (asset.temperature != null && asset.lastUpdated != null) {
        final day = DateFormat('MM-dd').format(asset.lastUpdated!);
        print('Processing - Date: $day, Asset ID: ${asset.id}, Temp: ${asset.temperature}');
    ...
    print('Unique days found: ${temperaturesByDay.keys.length}');
    print('Days: ${temperaturesByDay.keys.toList().join(", ")}');
